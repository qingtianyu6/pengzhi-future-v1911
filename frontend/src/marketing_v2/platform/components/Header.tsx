    ...
    <header className="site-header">
      <div className="site-header__inner">
        ...
        <nav className="top-nav" aria-label="主导航">
          {nav.map((label) => <button key={label} onClick={() => go(label)} className={label === '平台介绍' ? 'is-active' : ''}>{label}</button>)}
        </nav>
        <div className="header-actions">
          <button className="icon-button" aria-label="搜索" onClick={openSearch}><Icon name="search" /></button>
