    ...
    <header className="site-header">
      <div className="site-header__inner">
        ...
        <nav className="top-nav" aria-label="主导航">
          {nav.map((label) => <button key={label} onClick={() => go(label)} className={label === '应用场景' ? 'is-active' : ''}>{label}</button>)}
        </nav>
        <div className="header-actions">
          <button className="icon-button" aria-label="搜索" onClick={openSearch}><Icon name="search" /></button><span className="header-divider"/>
          <button className="outline-button compact" onClick={() => navigate('/platform?mode=guest')}><Icon name="user" />游客模式</button>
